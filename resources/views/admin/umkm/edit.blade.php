...
<div class="mb-8">
    ...
    <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Ubah Data Produk</h1>
    <p class="text-slate-500 mt-1">Perbarui informasi detail mengenai produk UMKM desa.</p>
</div>
...
<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-4xl">
    <form action="{{ route('admin.umkm.update', $umkm->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8 space-y-6">
        ...
        <!-- Informasi Produk -->
        <div class="space-y-4">
            <h3 class="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">Informasi Produk</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    <label for="kategori" class="block text-sm font-semibold text-slate-700 mb-1.5">Kategori Produk <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="harga" class="block text-sm font-semibold text-slate-700 mb-1.5">Harga (Rp) <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="unit" class="block text-sm font-semibold text-slate-700 mb-1.5">Satuan Harga <span class="text-red-500">*</span></label>
        ...
        <div class="space-y-4 pt-4">
            <h3 class="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">Informasi Penjual</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="nama_penjual" class="block text-sm font-semibold text-slate-700 mb-1.5">Nama Toko / Penjual <span class="text-red-500">*</span></label>
                ...
                <div>
                    <label for="no_whatsapp" class="block text-sm font-semibold text-slate-700 mb-1.5">Nomor WhatsApp <span class="text-slate-400 font-normal">(Opsional)</span></label>
                    <input type="text" name="no_whatsapp" id="no_whatsapp" value="{{ old('no_whatsapp', $umkm->no_whatsapp) }}" 
                           class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-600 focus:border-transparent outline-none transition-all">
                    <p class="text-xs text-slate-500 mt-1">Gunakan format awalan 62. Contoh: 62822...</p>
        ...
        <!-- Detail Tambahan -->
        <div class="space-y-4 pt-4">
            <h3 class="text-lg font-bold text-slate-900 border-b border-slate-100 pb-2">Detail Tambahan</h3>
            
            <div>
                <label for="deskripsi" class="block text-sm font-semibold text-slate-700 mb-1.5">Deskripsi Produk</label>
                <textarea name="deskripsi" id="deskripsi" rows="3" 
                          class="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-600 focus:border-transparent outline-none transition-all">{{ old('deskripsi', $umkm->deskripsi) }}</textarea>
        ...
        <div class="pt-6 border-t border-slate-100 flex items-center justify-end gap-3">
            <a href="{{ route('admin.umkm.index') }}" class="px-5 py-2.5 text-sm font-semibold text-slate-600 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors">Batal</a>
