...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Produk UMKM</h1>
        <p class="text-slate-500 mt-1">Kelola data produk unggulan warga untuk Pasar Desa.</p>
    </div>
            ...
            <tbody class="divide-y divide-slate-200">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    <td class="px-6 py-4 flex items-center gap-4">
                        @if($umkm->gambar)
                            <img src="{{ asset('storage/' . $umkm->gambar) }}" alt="{{ $umkm->nama_produk }}" class="w-14 h-14 rounded-lg object-cover border border-slate-200">
                        @else
                            <div class="w-14 h-14 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                            </div>
                        @endif
                        <div>
                            <p class="font-bold text-slate-900">{{ $umkm->nama_produk }}</p>
                            <p class="text-xs text-slate-500 line-clamp-1 max-w-[200px]">{{ $umkm->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                        </div>
                    </td>
                    <td class="px-6 py-4">
                        <p class="font-bold text-slate-900">{{ $umkm->nama_penjual }}</p>
                        <span class="inline-block mt-1 px-2.5 py-0.5 bg-green-50 text-green-700 font-semibold rounded-full text-[10px] uppercase tracking-wide border border-green-100">
                            {{ $umkm->kategori }}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <p class="font-bold text-green-800">Rp {{ number_format($umkm->harga, 0, ',', '.') }}</p>
                        <p class="text-xs text-slate-500">{{ $umkm->unit }}</p>
                    </td>
                ...
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <h3 class="font-bold text-slate-700 text-lg">Belum ada data UMKM</h3>
                            <p class="text-slate-500 mt-1 max-w-sm">Anda belum menambahkan produk jualan warga desa.</p>
