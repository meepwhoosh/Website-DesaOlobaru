    ...
    <div class="p-6 md:p-8">
        ...
        <div class="flex flex-col md:flex-row md:items-start justify-between gap-6 pb-6 border-b border-slate-100 dark:border-slate-700/50">
            <div class="space-y-4">
                ...
                <div class="flex items-center gap-4 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400">{{ $pesan->email }}</p>
                        @if($pesan->no_hp)
                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">HP: {{ $pesan->no_hp }}</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="shrink-0 flex flex-col gap-3">
                ...
                @php
                    $hasPhone = !empty($pesan->no_hp);
                    $waLink = $hasPhone ? 'https://wa.me/'.preg_replace('/[^0-9]/', '', $pesan->no_hp) : '#';
                @endphp
                
                <a href="{{ $hasPhone ? $waLink : '#' }}" target="{{ $hasPhone ? '_blank' : '' }}" class="inline-flex items-center justify-center px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-xl text-sm font-semibold transition-colors shadow-sm {{ !$hasPhone ? 'opacity-50 cursor-not-allowed' : '' }}" title="{{ !$hasPhone ? 'Tidak ada nomor HP' : '' }}">
                ...
                <a href="mailto:{{ $pesan->email }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold transition-colors shadow-sm">
