...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900 dark:text-white">Sejarah Desa</h1>
        <p class="text-slate-500 dark:text-slate-300 dark:text-white mt-1">Kelola data rekam jejak, asal-usul, dan peristiwa penting desa.</p>
    </div>
...
<form id="bulkDeleteForm" action="{{ route('admin.sejarah.bulk-destroy') }}" method="POST">
    @csrf
    @method('DELETE')
        ...
        <table class="w-full text-left border-collapse">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-white/10">
                ...
                <tr class="hover:bg-slate-50 dark:bg-[#0f0f0f] dark:bg-slate-800 dark:hover:bg-[#202020] transition-colors">
                    <td class="px-6 py-4">
                        <input type="checkbox" name="ids[]" value="{{ $sejarah->id }}" class="item-checkbox rounded border-slate-300 dark:border-slate-600 text-blue-600 shadow-sm focus:ring-blue-500 dark:bg-slate-700">
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4">
                        <p class="font-bold text-slate-900 dark:text-white">{{ $sejarah->judul }}</p>
                    </td>
                    <td class="px-6 py-4">
                        <p class="text-xs text-slate-500 dark:text-slate-300 dark:text-white line-clamp-2 leading-relaxed">{{ $sejarah->konten }}</p>
                    </td>
                ...
                <tr>
                    <td colspan="5" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <h3 class="font-bold text-slate-700 dark:text-slate-200 dark:text-white text-lg">Belum ada riwayat sejarah</h3>
                            <p class="text-slate-500 dark:text-slate-300 dark:text-white mt-1 max-w-sm">Anda belum menambahkan garis waktu (timeline) sejarah desa.</p>
