...
<div class="mb-8 flex items-center justify-between">
    <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Edit Data Penduduk</h1>
        <p class="text-slate-500 mt-1">Ubah informasi penduduk.</p>
    </div>
    <a href="{{ route('admin.data-desa.index') }}" class="text-slate-500 hover:text-slate-700 font-medium text-sm flex items-center gap-1">
...
<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden max-w-3xl">
    <form action="{{ route('admin.data-desa.update', $data->id) }}" method="POST" class="p-6 md:p-8">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <!-- NIK -->
            <div>
                <label for="nik" class="block text-sm font-semibold text-slate-700 mb-1.5">NIK / No. KTP</label>
            ...
            <!-- Jenis Kelamin -->
            <div>
                <label for="jenis_kelamin" class="block text-sm font-semibold text-slate-700 mb-1.5">Jenis Kelamin <span class="text-red-500">*</span></label>
            ...
            <div>
                <label for="tempat_tanggal_lahir" class="block text-sm font-semibold text-slate-700 mb-1.5">Tempat, Tanggal Lahir</label>
                <input type="text" name="tempat_tanggal_lahir" id="tempat_tanggal_lahir" value="{{ old('tempat_tanggal_lahir', $data->tempat_tanggal_lahir) }}"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-600 focus:ring focus:ring-green-600/20 transition-all outline-none text-slate-700"
            ...
            <!-- Pendidikan -->
            <div>
                <label for="pendidikan" class="block text-sm font-semibold text-slate-700 mb-1.5">Pendidikan Terakhir</label>
                <input type="text" name="pendidikan" id="pendidikan" value="{{ old('pendidikan', $data->pendidikan) }}"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-600 focus:ring focus:ring-green-600/20 transition-all outline-none text-slate-700"
            ...
            <!-- Pekerjaan -->
            <div>
                <label for="pekerjaan" class="block text-sm font-semibold text-slate-700 mb-1.5">Pekerjaan</label>
            ...
            <div class="md:col-span-2">
                <label for="dusun" class="block text-sm font-semibold text-slate-700 mb-1.5">Dusun / Alamat</label>
                <input type="text" name="dusun" id="dusun" value="{{ old('dusun', $data->dusun) }}"
                    class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:border-green-600 focus:ring focus:ring-green-600/20 transition-all outline-none text-slate-700"
