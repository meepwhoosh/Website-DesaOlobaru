...
<div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl md:text-3xl font-bold text-slate-900">Data Penduduk Desa</h1>
        <p class="text-slate-500 mt-1">Kelola arsip data penduduk dari file Excel.</p>
    </div>
...
<!-- Statistik Singkat -->
<div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm mb-6 flex items-center gap-6">
    ...
    <div>
        <p class="text-sm font-semibold text-slate-500 uppercase tracking-widest mb-1">Total Data Masuk di Database</p>
        <h3 class="text-3xl font-black text-slate-800">{{ number_format($dataDesaCount, 0, ',', '.') }} <span class="text-base font-medium text-slate-500 normal-case">Jiwa</span></h3>
    </div>
</div>
...
<div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
    <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
        <h2 class="text-lg font-bold text-slate-800">Arsip File Excel Penduduk</h2>
        <p class="text-sm text-slate-500 mt-1">Daftar file rekapitulasi data penduduk yang telah tersimpan di sistem.</p>
    </div>
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="font-bold text-slate-900">{{ $file['name'] }}</span>
                        </div>
                    </td>
                ...
                <tr>
                    <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                        <svg class="mx-auto h-12 w-12 text-slate-300 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
...
<div id="importModal" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <h3 class="text-lg font-bold text-slate-900">Import Data dari Excel</h3>
        ...
        <form action="{{ route('admin.data-desa.import') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="p-6 space-y-4">
                <p class="text-sm text-slate-600">Unggah file Excel untuk memperbarui database. Jika Anda mengunggah nama/NIK yang sudah ada, data lama akan ditimpa (di-update).</p>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Pilih File Excel (.xlsx, .xls, .csv)</label>
                    <input type="file" name="file_excel" accept=".xlsx,.xls,.csv" required class="w-full text-sm text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-sm file:font-semibold file:bg-green-50 file:text-green-700 hover:file:bg-green-100 border border-slate-200 rounded-xl">
                </div>
            </div>
            <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onclick="document.getElementById('importModal').classList.add('hidden')" class="px-4 py-2 text-sm font-semibold text-slate-600 hover:text-slate-800 transition-colors">Batal</button>
