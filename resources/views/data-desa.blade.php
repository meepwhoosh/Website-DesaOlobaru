...
<section class="py-12 bg-slate-50/50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div class="mb-2">
            ...
            <p class="text-slate-500 text-sm mt-1">Ringkasan data kependudukan Desa Olobaru</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-6">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Penduduk</p>
                    <h3 class="text-3xl font-black text-slate-800">{{ number_format($totalPenduduk, 0, ',', '.') }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-6">
                <div class="w-16 h-16 rounded-2xl bg-[#e7f5ff] text-[#339af0] flex items-center justify-center shrink-0">
                    <!-- Male Icon -->
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 14a5 5 0 100-10 5 5 0 000 10zm0 0v7m-4-4h8"></path>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.5 8.5l5-5m0 0v4m0-4h-4" />
                    </svg>
                </div>
                <div>
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Laki-Laki</p>
                    <h3 class="text-3xl font-black text-slate-800">{{ number_format($lakiLaki, 0, ',', '.') }}</h3>
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-6">
                ...
                <div>
                    <p class="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Perempuan</p>
                    <h3 class="text-3xl font-black text-slate-800">{{ number_format($perempuan, 0, ',', '.') }}</h3>
                </div>
            </div>
        </div>
        ...
        <div class="mt-10">
            <h3 class="text-lg font-bold text-slate-800 flex items-center gap-2 mb-4 border-b border-slate-200 pb-3">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h4 class="text-base font-bold text-slate-800 mb-6">Tren Kelompok Umur</h4>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h4 class="text-base font-bold text-slate-800 mb-6 text-center">Status Perkawinan</h4>
                    <div class="relative h-40 w-full flex items-center justify-center max-w-sm mx-auto">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
                    <div class="flex justify-between items-center mb-6">
                        <h4 class="text-base font-bold text-slate-800">Distribusi Per Dusun</h4>
                        <span class="text-xs font-semibold text-slate-400">{{ array_sum($dusun->toArray()) }} Total Jiwa</span>
                    </div>
                ...
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm flex flex-col">
                    <div class="flex justify-between items-center mb-6">
                        <h4 class="text-base font-bold text-slate-800">Tingkat Pendidikan</h4>
                    </div>
        ...
        <div class="mt-8 pb-12">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <!-- Donut: Agama -->
                <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    <h4 class="text-base font-bold text-slate-800 mb-6 text-center">Agama</h4>
                    <div class="relative h-40 w-full flex items-center justify-center max-w-sm mx-auto">
                        <canvas id="chartAgama"></canvas>
                    </div>
                </div>

                <!-- Table: Pekerjaan (Scrollable) -->
                <div class="bg-white p-0 rounded-2xl border border-slate-200 shadow-sm flex flex-col h-64 overflow-hidden">
                    <div class="p-4 bg-slate-400 text-white flex justify-between items-center shrink-0">
                        <h4 class="text-sm font-bold tracking-wide">Jenis Pekerjaan</h4>
                        <span class="text-sm font-bold pr-2">Jumlah</span>
                    </div>
                    <div class="overflow-y-auto flex-grow custom-scrollbar bg-white">
                        <table class="w-full text-left text-sm">
                            <tbody class="divide-y divide-slate-100">
                                @php
                                    $sortedPekerjaan = collect($pekerjaan)->sortDesc();
                                @endphp
                                @foreach($sortedPekerjaan as $nama => $jumlah)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="py-3 px-4 text-slate-700 font-medium">{{ $nama ?: 'Tidak Diketahui' }}</td>
                                    <td class="py-3 px-4 text-slate-900 font-bold text-right pr-6">{{ number_format($jumlah, 0, ',', '.') }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const agamaRawData = {!! json_encode($agama->filter(fn($val, $key) => trim((string)$key) !== '' && trim((string)$key) !== '-')->toArray()) !!};
        ...
        const statusLabelsRaw = {!! json_encode(array_keys($status_perkawinan->toArray())) !!};
        const statusData = {!! json_encode(array_values($status_perkawinan->toArray())) !!};
        const statusLabels = statusLabelsRaw.map((lbl, i) => lbl + ' (' + statusData[i] + ')');
        
        const umurLabelsRaw = {!! json_encode(array_keys($umur)) !!};
        const umurData = {!! json_encode(array_values($umur)) !!};
        const umurLabels = umurLabelsRaw.map((lbl, i) => lbl + ' (' + umurData[i] + ')');

        const dusunLabelsRaw = {!! json_encode(array_keys($dusun->toArray())) !!};
        const dusunData = {!! json_encode(array_values($dusun->toArray())) !!};
        const dusunLabels = dusunLabelsRaw.map((lbl, i) => lbl + ' (' + dusunData[i] + ')');

        const pendidikanLabelsRaw = {!! json_encode(array_keys($pendidikan->toArray())) !!};
        const pendidikanData = {!! json_encode(array_values($pendidikan->toArray())) !!};
        const pendidikanLabels = pendidikanLabelsRaw.map((lbl, i) => lbl + ' (' + pendidikanData[i] + ')');

        const pekerjaanLabelsRaw = {!! json_encode(array_keys($pekerjaan->toArray())) !!};
        const pekerjaanData = {!! json_encode(array_values($pekerjaan->toArray())) !!};
