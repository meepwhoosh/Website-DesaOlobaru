...
<div>
    ...
    <label for="{{ $id }}" id="dropzone_{{ $id }}" class="mt-1 flex flex-col justify-center min-h-[120px] rounded-xl border-2 border-dashed border-slate-300 px-4 py-3 hover:bg-slate-50 transition-colors relative cursor-pointer group">
        <div class="text-center w-full" id="upload_text_{{ $id }}" @if($currentImage) style="display:none;" @endif>
            <svg class="mx-auto h-8 w-8 text-slate-300 group-hover:text-[#2D5A27] transition-colors" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            <p class="text-[11px] leading-5 text-slate-500">{{ $helper }}</p>
        </div>

        <input id="{{ $id }}" name="{{ $name }}" type="file" class="sr-only" accept="image/*" {{ $required && !$currentImage ? 'required' : '' }}>
        ...
        <div id="preview_container_{{ $id }}" class="w-full flex flex-col items-center justify-center p-2 z-10" style="display: {{ $currentImage ? 'flex' : 'none' }};">
            <div class="relative w-full flex justify-center">
                <img id="preview_img_{{ $id }}" src="{{ $currentImage ? asset('storage/' . $currentImage) : '#' }}" alt="Preview" class="max-h-24 object-contain rounded-lg shadow-sm border border-slate-200">
                <!-- Remove Button -->
            ...
            <p id="filename_{{ $id }}" class="text-sm text-slate-600 font-medium truncate max-w-full mt-3 px-4">{{ $currentImage ? 'Gambar Tersimpan' : '' }}</p>
        </div>
    </label>
    @error($name)
